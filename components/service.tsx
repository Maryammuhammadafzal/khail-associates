...
const Service = ({ subheading, heading, paragraph, image, index, icon }: any) => {
    return (
        <section className="process-section dark:bg-neutral-900 relative  w-full h-auto min-h-screen flex  justify-center items-center">
            ...
            <div className={`${index % 2 == 0 ? "order-1" : "order-2"} relative w-[60%] h-screen flex justify-center items-start`}>
                <Image data-aos="zoom-in-up" src={image} alt='Architectural process illustration' width={800} height={400} className='h-full object-cover' />
            </div>
        </section>
    )
}
